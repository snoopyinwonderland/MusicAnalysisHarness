...
Motif occurrence extent MUST NOT be copied mechanically from a Phrase span. Phrase and subphrase boundaries MAY contribute extent evidence alongside beam, rest, metric, duration, tie, contour, arrival, and recurrence consistency. A bounded matching signature is relation evidence, not necessarily the complete Motif extent. Once a prototype occurrence is established from a local recurrence cycle or parallel-cell structure, its attack count SHOULD be projected to a distant recurrence. A duration-only difference at the final event MAY retain the same Motif variant identity when interval sequence, contour, and event coverage remain equivalent. Detailed rules are defined in `MOTIF_VARIATION_AND_EXTENT_SPEC_v0.1.md`.
